...
const IndividualDeactivatedUser = () => {
    const { email } = useParams();
    const [user, setUser] = useState([]);
    const [loading, setLoading] = useState(false);
    const fetchUserData = async (email) => {
        try {
          const userData = await userService.fetchDeactivatedIndividualUserData(email);
          setUser(userData);
        } catch (error) {
   ...
   <table style={{ width: 'auto' }}>
     <tbody>
       ...
       <tr  >
         <td style={{ width: 'auto'  }}><strong>Reason for Deactivation</strong></td>
         <td>:</td>
         <td>{user.reason}</td>
       </tr>
     </tbody>
...
export default IndividualDeactivatedUser;
